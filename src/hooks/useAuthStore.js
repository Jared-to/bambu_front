...
export const useAuthStore = () => {
  ...
  const startLogin = async (formulario) => {
    ...
    try {
      ...
      dispatch(onLogin({ name: data.fullName, id: data.id, rol: data.roles[0], foto: data.foto }));
      sessionStorage.setItem('userId', data.id);
      sessionStorage.setItem('email', data.email);
      sessionStorage.setItem('token', data.token);
      if (data?.almacen?.id) {
        sessionStorage.setItem('sucursal', data?.almacen?.id);
      }
  ...
  const checkAuth = async () => {
    ...
    try {
      ...
      sessionStorage.setItem('userId', data.id);
      sessionStorage.setItem('token', data.token);
      sessionStorage.setItem('email', data.fullName);

      return dispatch(onLogin({ name: data.fullName, id: data.id, rol: data.roles[0], foto: data.foto }));
